...
var Dashboard = function () {
    ...
    var _AppSalesLinesChart = function(element, height) {
        ...
        if($(element).length > 0) {
            ...
            d3.csv(site_url()+'global_assets/demo_data/dashboard/app_sales.csv', function(error, data) {
                formatted = data;
                console.log(data)
                redraw();
            });
    ...
    var _checksFrequency= function(element, height, color) {
        if (typeof d3 == 'undefined') {
            console.warn('Warning - d3.min.js is not loaded.');
            return;
        }
 d3.select(element).selectAll('*').remove();
        ...
        if($(element).length > 0) {
            ...
            // ------------------------------
                ...
                var m = $('#month_freq').val()
                var y2 = $('#year_freq').val()
            d3.json(site_url()+'Dashboard/checksSubmissionFrequency/'+y2+'-'+m, function (error, data) {
                ...
                function mousemove() {
                    ...
                    if(mousex >= (d3Container.node().getBoundingClientRect().width - focusText.select('text').node().getBoundingClientRect().width - margin.right - margin.left)) {
                        focusText.select('text').attr('text-anchor', 'end').attr('x', function () { return (x(d.date) - 15) + 'px' }).text(formatDate(d.date) + ' - ' + d.value + ' Submitted');
                    }
                    else {
                        focusText.select('text').attr('text-anchor', 'start').attr('x', function () { return (x(d.date) + 15) + 'px' }).text(formatDate(d.date) + ' - ' + d.value + ' Submitted');
                    }
                }
    ...
    var _MonthlySalesAreaChart = function(element, height, color) {
        ...
        if($(element).length > 0) {
            ...
            d3.json(site_url()+'global_assets/demo_data/dashboard/monthly_sales.json', function (error, data) {
    ...
    var _NumberOfHEIDonutChart = function(element, size) {
        if (typeof d3 == 'undefined') {
            console.warn('Warning - d3.min.js is not loaded.');
            return;
        }
        ...
        if($(element).length > 0) {
            ...
            $.ajax({
                    method: "POST",
                    url: site_url()+"Dashboard/stat_countHEI",
                      cache: false,
                    contentType: false,
                    processData: false,
...
$('.heicount_stats').find('.total').text(sum)
$('.heicount_stats').find('.description').text("Number of HEI")
            ...
            var tip = d3.tip()
                ...
                .html(function (d) {
                    return '<ul class="list-unstyled mb-1">' +
                        '<li>' + '<div class="font-size-base mb-1 mt-1">' + d.data.icon + d.data.HEIType + '&nbsp(' + (100 / (sum / d.value)).toFixed(2) + '%)</div> </li>' +
                        d.data.html+
  ...
  var GraduateCountDonutCHART = function(element,size,text,pl) {
    if (typeof d3 == 'undefined') {
        console.warn('Warning - d3.min.js is not loaded.');
        return;
    }
    ...
    if ($(element).length > 0) {
        ...
        var sy = $('#programyear_gr').val();
      var endpoint ="stat_numb_graduate/" + sy ;
 if(pl){
          
          endpoint ="stat_numb_graduate_proglevel/" + sy +"/"+pl;
      }

        $.ajax({
            method: "POST",
            url: site_url() + "Dashboard/" +endpoint,
            cache: false,
            contentType: false,
            processData: false,
            success: function(data) {
                ...
                // Remove existing SVG to prevent appending multiple charts
                d3Container.select('svg').remove();

                // Tooltip
                
                var currency = sum.toLocaleString('en-US');
                $(element).parent('.graph').find('.total').text(currency);
                $(element).parent('.graph').find('.description').text(text);
                ...
                var tip = d3.tip()
                    ...
                    .html(function(d) {
                        return '<ul class="list-unstyled mb-1">' +
                            '<li>' + '<div class="font-size-base mb-1 mt-1">' + d.data.icon + d.data.category + '&nbsp(' + (100 / (sum / d.value)).toFixed(2) + '%)</div> </li>' +
                            d.data.html +
                            '</ul>';
                    });
  ...
  var EnrollmentCountDonutCHART = function(element,size,text,pl) {
    if (typeof d3 == 'undefined') 
    {
        console.warn('Warning - d3.min.js is not loaded.');
        return;
    ...
    if ($(element).length > 0) {
      ...
      var sy = $('#programyear_en').val();
      var endpoint = "stat_numb_enrollment/" + sy;
      
      if(pl){
          
          endpoint ="stat_numb_enrollment_proglevel/" + sy +"/"+pl;
      }
  ...
  var ProgramCountDonutCHART = function(element, size,text,pl) {
    if (typeof d3 == 'undefined') {
        console.warn('Warning - d3.min.js is not loaded.');
        return;
    }
    ...
    if ($(element).length > 0) {
        ...
        var sy = $('#programyear').val();
  var endpoint = "stat_numb_program/" + sy;
      if(pl){
          
          endpoint ="stat_numb_program_proglevel/" + sy +"/"+pl;
      }
        $.ajax({
            method: "POST",
            url: site_url() + "Dashboard/" +endpoint,
    ...
    return {
        ...
        initProgramCharts: function() {
            
           
            
     
            ProgramCountDonutCHART('#program', 150,"Total Enrollment",false);
            ProgramCountDonutCHART('#program_prebac', 70,"Pre-Baccalaureate",40);
            ProgramCountDonutCHART('#program_bachelor', 70,"Baccalaureate",50);
            ProgramCountDonutCHART('#program_postbac', 70,"Post-Baccalaureate",70);
            ProgramCountDonutCHART('#program_master', 70,"Masters",80);
            ProgramCountDonutCHART('#program_doctor', 70,"Doctoral",90);
            
            
        },
        initEnrollmentCharts: function() {
       
           
           
            EnrollmentCountDonutCHART('#enrollment', 150,"Total Enrollment",false);
            
            EnrollmentCountDonutCHART('#enrollment_prebac', 70,"Pre-Baccalaureate",40);
            EnrollmentCountDonutCHART('#enrollment_bachelor', 70,"Baccalaureate",50);
            EnrollmentCountDonutCHART('#enrollment_postbac', 70,"Post-Baccalaureate",70);
            EnrollmentCountDonutCHART('#enrollment_master', 70,"Masters",80);
            EnrollmentCountDonutCHART('#enrollment_doctor', 70,"Doctoral",90);
   
        },
        initGraduateCharts: function() {
       
          
           
            GraduateCountDonutCHART('#graduate', 150,"Total Graduate",false);
            GraduateCountDonutCHART('#graduate_prebac', 70,"Pre-Baccalaureate",40);
            GraduateCountDonutCHART('#graduate_bachelor', 70,"Baccalaureate",50);
            GraduateCountDonutCHART('#graduate_postbac', 70,"Post-Baccalaureate",70);
            GraduateCountDonutCHART('#graduate_master', 70,"Masters",80);
            GraduateCountDonutCHART('#graduate_doctor', 70,"Doctoral",90);
   
        },
        initChecks: function() {
       
           _checksFrequency('#checksFrequency', 100, '#4DB6AC');
        ...
        initCharts: function() {
         ...
         //   _MonthlySalesAreaChart('#monthly-sales-stats', 100, '#4DB6AC');
            ...
            // Donut charts
            _NumberOfHEIDonutChart('#campaigns-donut', 175);
...
document.addEventListener('DOMContentLoaded', function() {
         ...
         var year = new Date().getFullYear();
            ...
            $('#programyear').val((year-1)+'-'+year);
            $('#programyear_en').val((year-1)+'-'+year);
            $('#programyear_gr').val((year-1)+'-'+year);
    ...
    Dashboard.initProgramCharts();
    
    
    Dashboard.initEnrollmentCharts();
    
    Dashboard.initGraduateCharts();
    }, 3000);
    
    Dashboard.initChecks();
});
...
$(document).ready(function(){
    ...
    $('#programyear').change(function(){
        ...
        $('.AcadYear').html("&nbsp;&nbsp;Academic Year:&nbsp;<b>"+$(this).val()+"</b>")
        Dashboard.initProgramCharts();
    })
    $('#programyear_en').change(function(){
       
        $('.AcadYear').html("&nbsp;&nbsp;Academic Year:&nbsp;<b>"+$(this).val()+"</b>")
        Dashboard.initEnrollmentCharts();
    })
    $('#programyear_gr').change(function(){
      ...
      $('#month_freq').change(function(){
           Dashboard.initChecks();
           
       })
      

      $('#year_freq').change(function(){
              Dashboard.initChecks();
       })
